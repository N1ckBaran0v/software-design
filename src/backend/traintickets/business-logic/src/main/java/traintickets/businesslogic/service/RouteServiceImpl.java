...
public final class RouteServiceImpl implements RouteService {
    ...
    public List<Route> getRoutes(Filter filter) {
        ...
        var races = StreamSupport.stream(raceRepository.getRaces(systemRole, filter).spliterator(), false)
                .filter(race -> isCorrect(race, filter)).toList();
    ...
    private boolean isCorrect(Race race, Filter filter) {
    ...
    public List<List<Place>> getFreePlaces(RaceId raceId) {
        ...
        railcarRepository.getRailcarsByTrain(systemRole, train.id()).forEach(railcar -> railcars.put(railcar.id(), railcar));
        var boughtAll = new ArrayList<Set<Integer>>(numbers.size());
        for (var i = 0; i < numbers.size(); ++i) {
            boughtAll.add(new HashSet<>());
        }
        ticketRepository.getTicketsByRace(systemRole, raceId).forEach(
